 ...
 *       - 다음 사용자 인증 정보로 실행: "나"        ← 반드시 먼저 이 값으로
 *       - 액세스 권한이 있는 사용자:   "모든 사용자"  ← 그 다음 이 값으로
 *
 *     [자주 겪는 함정]
 *     실행 계정을 "웹 앱을 액세스하는 사용자"로 두면
 *     액세스 권한 목록에 "모든 사용자"가 아예 나타나지 않고
 *     "Google 계정이 있는 모든 사용자"까지만 고를 수 있습니다.
 *     이 상태로 배포하면 방문자가 로그인 화면으로 밀려나
 *     웹사이트에서 "전송에 실패했어요"가 뜹니다.
 *     실행 계정을 "나"로 바꾸면 "모든 사용자"가 나타납니다.
 *
 *     실행 계정을 "나"로 두어도 시트가 공개되지는 않습니다.
 *     방문자 대신 스크립트가 내 권한으로 기록할 뿐입니다.
 *
