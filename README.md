...
정신질환 치료가 아니라 번아웃, 취업·진로, 감정 돌봄, 자기 이해처럼
...
| 파일 | 내용 |
| --- | --- |
| `index.html` | 히어로, 고민 체크리스트, **프로그램 추천 진단**, 프로그램 7종, 진행 4단계, 후기, FAQ |
| `programs.html` | 프로그램별 주차 커리큘럼, 단회 세션, 이용료, 환불 규정 |
| `trial.html` | 프로그램 체험단 모집 안내 |
| `about.html` | 상담소 이야기, 네 가지 약속, 오시는 길 |
| `apply.html` | 첫 상담 신청 폼 (인라인 검증 + 완료 화면, `?trial=1` 로 체험단 신청) |
...
```
index.html  programs.html  trial.html  about.html
apply.html  privacy.html  404.html
...
backend/
  google-apps-script.gs — 신청서를 구글 시트에 기록하는 서버 코드
tools/
  check-endpoint.html   — 신청 폼 연결 상태를 점검하는 진단 페이지
```
...
**컨셉** — 로고에서 뽑은 코랄·파랑·초록에 따뜻한 아이보리 배경.
병원을 연상시키는 차가운 회색은 쓰지 않았습니다.

**핵심 메시지** — &ldquo;나를 알아가고, 함께 나아가는 시간&rdquo;
부정을 지우는 방식(&ldquo;아프지 않아도&rdquo;) 대신, 로고 슬로건의 두 동사
&lsquo;잇다·나아가다&rsquo;를 그대로 가져와 능동적으로 표현했습니다.

색상과 간격은 `style.css` 상단 `:root` 토큰만 바꾸면 전체에 반영됩니다.

```css
--coral: #f4826b;   /* 로고 가운데 하트 · 주요 행동 유도 */
--blue:  #5b8fcf;   /* 로고 왼쪽 사람 */
--green: #8cc56e;   /* 로고 오른쪽 사람 */
--sun:   #f5c34e;   /* 따뜻함을 더하는 보조색 */
```

색은 프로그램 하나가 아니라 주제 갈래에 붙습니다. 추천 진단 결과와 프로그램 카드의
색이 서로 맞도록 고정해 두었습니다.

| 색 | 주제 | 프로그램 |
| --- | --- | --- |
| 코랄 | 회복 · 휴식 | 쉼표 프로젝트, 스트레스 케어 클래스 |
| 파랑 | 방향 · 집중 | 방향 찾기 세션, 뇌파 리듬 트레이닝 |
| 햇살 | 감정 | 감정세탁소 |
| 초록 | 관계 | 마음 이음 서클, 연애애착유형검사 |

같은 색 카드가 2단 그리드에서 나란히 놓이지 않도록 순서를 잡아 두었습니다.
...
```json
{
  ...
  "연령대": "만 27-30세",
  "현재 상황": "직장인 1-3년 차",
  "관심 주제": "번아웃 회복, 자기 이해",   // 체험단 신청이면 앞에 [체험단] 이 붙습니다
  "상담 방식": "대면",
  "편한 시간대": "평일 저녁",
  "하고 싶은 이야기": "요즘 너무 지쳐요.",
  "청년 할인 안내": "희망",
...
- [x] 신청 폼을 구글 스프레드시트에 연결 (Apps Script)
- [x] 주소와 이메일을 실제 정보로 교체
- [ ] `privacy.html`의 개인정보 보호 책임자를 실제 담당자 성명으로 교체
- [ ] 후기 3건을 실제 동의받은 내용으로 교체 — 지금 문구는 예시입니다
- [ ] 통계 수치(1,280+ / 94%)를 실제 데이터로 교체
- [ ] 이용료 금액 확정 — 1:1 6만 원, 그룹 3.5만 원은 아직 확인되지 않은 예시입니다
- [ ] 뇌파 리듬 트레이닝 운영 요건 확인 — 국내에서 뇌파계는 의료기기로 분류됩니다.
      사용 장비의 인증 구분과 홍보 문구 범위를 미리 확인해 주세요.
      (사이트 문구는 진단·치료 표현 없이 &lsquo;훈련&rsquo;으로만 적어 두었습니다)
